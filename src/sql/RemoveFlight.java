...
//	 SELECT * FROM `la_schedule` WHERE start_date >'2012-11-18';
	...
	public ArrayList<Flights> rmFlight(Flights model, int selected) {

		
		for(int i = 0; i < model.getAllFlights().size(); i++){
			model.getAllFlights().get(i).getFlightId();
		}
	    ...
	    System.out.println(sdf.format(date));
			...
			try
			{
				...
				st.setInt(1,model.getAllFlights().get(selected).getFlightId());
				st.executeUpdate();
				
				
				
				PreparedStatement st1 = conn.prepareStatement("SELECT * FROM Flights WHERE departure_date >= " + sdf.format(date));
				ResultSet r1=st1.executeQuery();
				
				model.getAllFlights().clear();
				...
				while(r1.next()) 
				{
					...
					flight.getAllFlights().add(flight);
				}
			}
